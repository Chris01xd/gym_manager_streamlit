# app/pages/02_Ventas.py
# ──────────────────────────────────────────────────────────────────────────────
# Bootstrap de imports: asegura que el raíz del proyecto esté en sys.path
from pathlib import Path
import sys

# Si este archivo está en /<ROOT>/app/pages/02_Ventas.py, el raíz es parents[2]
...
# Imports de tu proyecto (ya deberían resolver)
...
def _to_datetime(v: Any) -> Optional[datetime]:
    ...
    if not s:
        return None
    # Maneja '2025-09-10 14:03:22', '2025-09-10T14:03:22', con o sin 'Z'
    s = s.replace("Z", "")
    try:
        return datetime.fromisoformat(s)
    except Exception:
        # último intento: formatos comunes
        for fmt in ("%Y-%m-%d %H:%M:%S", "%Y-%m-%d"):
            try:
                dt = datetime.strptime(s, fmt)
                # si venía solo fecha, poner 00:00
                if fmt == "%Y-%m-%d":
                ...
                if confirmar:
                    try:
                        with db_cursor(commit=True) as cur:
                            ...
                            # 2) Ítems con validación de stock
                            for it in items:
                        ...
                        # Limpiar carrito y ofrecer nueva venta
                        st.session_state["venta_items"] = []
...
with tab_listado:
    ...
    # Normaliza fechas a datetime para evitar formateos que fallen
    for v in ventas or []:
